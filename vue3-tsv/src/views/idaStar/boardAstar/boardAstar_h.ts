...
export class BoardAstar_h {
  ...
  *execStep() {
  ...
  async exec(stepCb?: (str: string) => void) {
    ...
    const stateCnt =
      this.openQueue.size() +
      Object.keys(this.closeSet).length +
      this.remoceCnt;
    console.log(
      `Done:还原路径${path.length}步,遍历状态${(stateCnt / 10 ** 6).toFixed(
        3
      )}M,清理内存${this.remoceCnt}条,耗时${(duration / 1000).toFixed(
        3
...
export class BoardAstar_hp {
  openQueue: HeapPlus<State2> = new HeapPlus((a, b) => a.cost - a.cost); //优先级列表
  ...
  async exec(stepCb?: (str: string) => void) {
    ...
    for (; this.openQueue.size(); ) {
      ...
      const child = this.board.getCanActoinDir(
        this.board.emptyIndex,
        stateobj.beforeState ? stateobj.action : undefined
      );
      for (const v of child) {
        const list = this.board.doAction(v, false);
        const nstateStr = list.join(",");
